Function Get-WGPath {
    ...
    $currentUser =  [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value # get account running function
    Write-Verbose "[$((Get-Date).TimeofDay)] Running as $currentUser"
    # S-1-5-18 is SYSTEM account for Windows
    if ($currentUser -eq "S-1-5-18") {
